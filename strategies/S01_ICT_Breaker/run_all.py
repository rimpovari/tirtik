"""
strategies/S01_ICT_Breaker/run_all.py
=======================================
30 parite x 7 TF seti x 5 RR degeri = 1050 backtest
...
Kullanim:
    python strategies/S01_ICT_Breaker/run_all.py
"""
...
def _combo_key(htf: str, ltf: str, rr: float) -> str:
    return f"{htf}_{ltf}_RR{rr:.0f}"
...
def _stats_row(symbol: str, htf: str, ltf: str, rr: float, stats: dict) -> dict:
    return {
        "Sembol":       symbol,
        "HTF":          htf,
        "LTF":          ltf,
        "Min_RR":       rr,
        "Combo":        _combo_key(htf, ltf, rr),
...
def write_symbol_excel(symbol: str, combo_results: dict) -> None:
    """
    combo_results: {combo_key: (trades_list, stats_dict)}
    Ozet sayfasi + her kombo icin ayri trade sayfasi.
    """
    ...
    for key, (_, stats) in combo_results.items():
        if not stats:
            continue
        htf, ltf, rr_str = key.split("_")[0], key.split("_")[1], key.split("_RR")[1]
        ozet_rows.append({
            "Kombinasyon":  key,
            "HTF":          htf,
            "LTF":          ltf,
    ...
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        if ozet_rows:
            ...
            _format_ozet_sheet(writer.sheets["Ozet"], ozet_df)

        # Trade detay sayfasi (her kombo)
        for key, (trades, stats) in combo_results.items():
            if not trades:
                continue
            sheet_name = key[:31]  # Excel 31 karakter siniri
            df = _trades_to_df(trades)
            df.to_excel(writer, sheet_name=sheet_name, index=False)
            _format_trades_sheet(writer.sheets[sheet_name], df)


def _format_ozet_sheet(ws, df: pd.DataFrame) -> None:
    try:
...
def write_master_excel(all_rows: list[dict]) -> None:
    ...
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        ...
        # 2. WinRate matrisi
        _pivot_sheet(writer, df, "WinRate", "WinRate_Matrix",
                     lambda v: f"{v:.1%}")

        # 3. TotalR matrisi
        _pivot_sheet(writer, df, "TotalR", "TotalR_Matrix")

        # 4. ProfitFactor matrisi
        _pivot_sheet(writer, df, "ProfitFactor", "PF_Matrix")

        # 5. En iyi 30 kombinasyon
        ...
        # 6. Sembol bazli ozet (her sembol icin en iyi kombo)
        best = (
...
def _pivot_sheet(writer, df: pd.DataFrame, value_col: str,
                 sheet_name: str, fmt=None) -> None:
    try:
        pivot = df.pivot_table(
            index="Sembol", columns="Combo",
...
def main() -> None:
    ...
    total   = len(SYMBOLS) * len(TF_SETS) * len(RR_VALUES)
    done    = 0
    skipped = 0
    t_start = time.time()
    all_rows: list[dict] = []
    ...
    print(f"\n{'='*60}")
    print(f"  ICT Breaker — Coklu Parametre Testi")
    print(f"  {len(SYMBOLS)} parite x {len(TF_SETS)} TF seti x {len(RR_VALUES)} RR = {total} kombo")
    print(f"{'='*60}\n")
    ...
    for symbol in SYMBOLS:
        ...
        for htf, ltf in TF_SETS:
            for rr in RR_VALUES:
                ...
                key  = _combo_key(htf, ltf, rr)
                pct  = done / total * 100

                try:
                    trades, stats = run_single(symbol, htf, ltf, rr)
                except Exception:
                    trades, stats = [], {}
                    skipped += 1
                ...
                if stats:
                    all_rows.append(_stats_row(symbol, htf, ltf, rr, stats))
                    wr = stats.get("win_rate", 0)
                    tr = stats.get("total_r", 0)
                    n  = stats.get("total_trades", 0)
                    print(f"  [{done:>4}/{total}] {pct:4.0f}%  "
                          f"{symbol:<8} {htf}/{ltf:<4} RR{rr:.0f}  "
                          f"{n:>3} islem  WR {wr:.0%}  R {tr:+.1f}")
                else:
                    print(f"  [{done:>4}/{total}] {pct:4.0f}%  "
                          f"{symbol:<8} {htf}/{ltf:<4} RR{rr:.0f}  -- sinyal yok --")

        # Bu sembol bitti, Excel yaz
