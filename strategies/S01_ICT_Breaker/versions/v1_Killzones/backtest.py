"""
V1 - Killzones (Time-Based Filtering)
======================================
Base strateji üzerine eklenen tek değişiklik:
MSS (Market Structure Shift) onayı yalnızca London veya New York
killzone saatleri içinde gerçekleşirse geçerli sayılır.

Base'den farklı olan: scan_ltf_for_breaker (killzone filtresi eklendi)
...
VERSION = "V1_Killzones"

# London: 07:00-11:00 UTC  |  New York: 13:00-17:00 UTC
...
def scan_ltf_for_breaker(ltf_window, direction: str) -> dict | None:
    """
    Base ile aynı mantık; fark: MSS mumunun killzone içinde olması zorunlu.
    """
    if len(ltf_window) < 6:
        return None
    ...
    if direction == "bull":
        for sl_idx in _swing_lows_idx(ltf_window):
            ...
            mss_time = None
            for ts, row in post.iloc[sweep_pos + 1:].iterrows():
                if row["close"] > bounce_high:
                    if _in_killzone(ts):          # ← KİLLZONE FİLTRESİ
                        mss_time = ts
                    break
            if mss_time is None:
                continue
